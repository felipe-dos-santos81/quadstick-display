...
from quadstick_display.model import (  # noqa: E402
    Binding,
    InvalidProfile,
    InvalidProfileName,
    Profile,
...
from quadstick_display.view import ProfileRenderer  # noqa: E402
...
class TestSuccessfulShow:
    ...
    def test_the_renderers_own_frame_object_reaches_the_device(self, controller, renderer, device):
        controller.show_profile('first.csv')
    ...
    def test_successive_shows_replace_the_current_profile(self, controller, device):
        ...
        assert controller.status == DisplayStatus(current_profile='second.csv', last_error=None)
...
class TestModelFailures:
    ...
    def test_unsafe_names_raise_the_typed_error(self, controller, renderer, device, name):

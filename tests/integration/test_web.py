...
class TestRender:
    ...
    def test_render_updates_the_selected_status_on_the_index(self, client, tmp_path):
        install_profile(tmp_path)
    ...
    def test_render_without_selection_is_400_and_draws_nothing(self, client, display):
    ...
    def test_render_missing_profile_is_404(self, client, app, display):
        response = client.post("/render", data={"selected_file": "does_not_exist.csv"})
    ...
    def test_hardware_failure_is_503_and_preserves_the_profile(self, tmp_path):
        ...
        display.armed = True
        response = client.post("/render", data={"selected_file": "alan_wake_II.csv"})
...
class TestMainComposition:
    def test_main_initializes_shows_startup_then_serves(self, monkeypatch):
        ...
        monkeypatch.setattr(main_module, "_get_local_ip_address", lambda: "192.0.2.1")
        run_calls = []
        ...
        assert run_calls == [{"host": "0.0.0.0", "port": 8080, "use_reloader": False}]
        assert "waveshare_epd" not in sys.modules

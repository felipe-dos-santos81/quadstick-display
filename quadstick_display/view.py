...
class _MouthpiecePainter:
    ...
    def __init__(
        self, image_blk, draw_blk, draw_red, arrow_left, arrow_right, font_text, x, y
    ):
        ...
        self.x = int(x)
        self.y = int(y)
        self.radius = ceil(TEXT_SIZE / TEXT_SIZE_OFFSET)
    ...
    def draw_arrow(self, is_puff):
        ...
        logger.debug(f"Drawing sip/puff arrow at {self.x}, {self.y}")
        self.image_blk.paste(im=arrow, box=(self.x - self.radius, self.y - self.radius))
    ...
    def draw_text(self, text, draw=None):
        ...
        y_text = int(self.y)
        radius_offset = ceil(self.radius / TEXT_SIZE_OFFSET)
    ...
    def draw(self, button):
        if button not in MP_BUTTONS:
            logger.debug(f"Button {button} is not a mouthpiece button")
            self.draw_text(button)
            return

        left, center, right, is_puff, is_soft = MP_BUTTONS[button]
        logger.debug(f"Drawing mouthpiece button {button}")
...
class ProfileRenderer:
    ...
    def __init__(self, resources: Path):
        ...
        self.arrow_left_icon = self._load_icon(
            images / IMAGE_ARROW, flip_horizontal=True
        )
    ...
    def render(self, profile: Profile, size: DisplaySize) -> DisplayFrame:
        ...
        if draw_height > height:
            logger.warning(f"Resizing image to {width}x{draw_height} to fit all data")
        else:
            draw_height = height
        ...
        logger.info("Drawing Quadstick settings table")
        ...
        if draw_height > height:
            logger.info(f"Resizing image back to the display size {width}x{height}")
    ...
    def render_startup(self, url: str, size: DisplaySize) -> DisplayFrame:
        ...
        logger.debug("Rendering startup screen")
